...
namespace DALSA.SaperaLT.Examples.NET.Utils
{
    public class ExampleUtils
    {
        ...
        static public bool GetParametersFrom3DBuffer(SapBuffer Buffer, ref int Invalid_Flag_C,
            ref double Invalid_Value_C, ref double Scale_A, ref double Offset_A, ref double Scale_C,
            ref double Offset_C, ref BUFFER_LAYOUT Buffer_Layout, ref SapBuffer.Val Output_Mode)
        {
            if (Buffer.Height > 1)
                Buffer_Layout = ExampleUtils.BUFFER_LAYOUT.Range;
            else
                Buffer_Layout = ExampleUtils.BUFFER_LAYOUT.Profile;

            if (!Buffer.GetParameter(SapBuffer.Prm.SCAN3D_INVALID_DATA_VALUE_C, out Invalid_Value_C))
                return false;

            if (!Buffer.GetParameter(SapBuffer.Prm.SCAN3D_INVALID_DATA_FLAG_C, out Invalid_Flag_C))
                return false;

            if (!Buffer.GetParameter(SapBuffer.Prm.SCAN3D_COORD_SCALE_A, out Scale_A))
                return false;

            if (!Buffer.GetParameter(SapBuffer.Prm.SCAN3D_COORD_OFFSET_A, out Offset_A))
                return false;

            if (!Buffer.GetParameter(SapBuffer.Prm.SCAN3D_COORD_SCALE_C, out Scale_C))
                return false;

            if (!Buffer.GetParameter(SapBuffer.Prm.SCAN3D_COORD_OFFSET_C, out Offset_C))
                return false;

            if (!Buffer.GetParameter(SapBuffer.Prm.SCAN3D_OUTPUT_MODE, out Output_Mode))
                return false;

            return true;
        }

        static public void DisplayRange(SapView View, SapBuffer Buffer)
        {
        ...
        static public bool GetSelectionFromConsoleCR(ref int pIndex_DisplayedComponent)
        {
            Console.WriteLine();
            Console.WriteLine("Select the component to display (or 'q' to quit)");
            Console.WriteLine();
            Console.WriteLine("..............................................");
            Console.WriteLine("1: Z (C)");
            Console.WriteLine("2: Reflectance (R)");

            ConsoleKeyInfo c = Console.ReadKey(true);
            if (!(c.Key == ConsoleKey.Q || c.KeyChar == '2' || c.KeyChar == '1'))
            {
                Console.WriteLine();
                Console.WriteLine("Invalid selection!");
            }
            else
            {
                ...
                {
                    pIndex_DisplayedComponent = c.KeyChar - '0';
                }
            }
            return true;
        }

        static public bool GetSelectionFromConsoleAC(ref int pIndex_DisplayedComponent)
        {
            Console.WriteLine();
            Console.WriteLine("Select the component to display (or 'q' to quit)");
            Console.WriteLine();
            ...
            ConsoleKeyInfo c = Console.ReadKey(true);
            if (!(c.Key == ConsoleKey.Q || c.KeyChar == '1' || c.KeyChar == '0'))
            {
                Console.WriteLine();
                Console.WriteLine("Invalid selection!");
            }
        ...
        static public bool GetSelectionFromConsoleACRW(ref int pIndex_DisplayedComponent)
        {
            Console.WriteLine();
            Console.WriteLine("Select the component to display (or 'q' to quit)");
            Console.WriteLine();
            ...
            ConsoleKeyInfo c = Console.ReadKey(true);
            if (!(c.Key == ConsoleKey.Q || c.KeyChar == '2' || c.KeyChar == '1' || c.KeyChar == '0'))
            {
                Console.WriteLine();
                Console.WriteLine("Invalid selection!");
            }
